...
After tag `0.8.0`: `webv` ships as a single-binary Web Validate-compatible CLI under `src/cmd/webv/` (flags `--url|-u --files|-f --loop|-l --threads|-t --random|-r --duration|-d --verbose|-v --version`, defaults `statusCode=200`/`contentType=application/json`, tab-delimited output with `# pass complete` heartbeats, exit 0 on signal). Loader picks YAML or JSON by extension. The same Dockerfile bakes both `/movies-api` and `/webv` plus the suites at `/webv-suites/`. `deploy/webv/` runs the load generator as a Deployment in the `movies` namespace pointing at `http://movies-api.movies.svc.cluster.local:8080` with `--loop --threads=2`; a sibling NetworkPolicy locks egress to DNS + movies-api on 8080 only. `make webv-install` installs the CLI into `~/go/bin`. New `src/webv/benchmark.yaml` has 200 expected-200 entries spanning `/api/{movies,actors,genres}` plus path-id and query-string variants. IMPL-README §12 documents the full five-step inner loop. **Session 9** picks Bench tuning — turn the runner into a p95 + 500 RPS gate.
